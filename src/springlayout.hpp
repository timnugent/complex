...
//           Andrew Lumsdaine
// Modified by Tim Nugent - added maxiter code
 ...
 * http://www.broadinstitute.org/~orzuk/matlab/matlab_bgl/libmbgl/layouts.cc
 */
  template <typename T = double>
  class layout_and_iteration_tolerance
  {
  public:
    layout_and_iteration_tolerance(const T& tolerance=0.001, int maxiter = 10000)
    ...
    detail::graph::edge_or_side<EdgeOrSideLength, T> edge_or_side_length)
  {
    typedef typename property_traits<WeightMap>::value_type weight_type;
    return kamada_kawai_spring_layout(g, position, weight, topology, edge_or_side_length,
                                      layout_and_iteration_tolerance<weight_type>(),
                                      weight_type(1.0), 
                                      get(vertex_index, g));
  }
} // end namespace boost
